...
#include <map>
#include <chrono>
#include "FilterTimeFrameSliceByTracking_GR.h"
...
using nestdaq::FilterTimeFrameSliceByTracking_GR;
namespace bpo = boost::program_options;

FilterTimeFrameSliceByTracking_GR::FilterTimeFrameSliceByTracking_GR()
...
bool FilterTimeFrameSliceByTracking_GR::ProcessSlice(TTF& tf)
{
...
int FilterTimeFrameSliceByTracking_GR::findWirenumber(const WireMapArray& wireMapArray, uint64_t geo, int ch, int *foundid, int *foundGeo, int &Geofield) {
...
std::vector<std::vector<std::pair<int, int>>> FilterTimeFrameSliceByTracking_GR::clusterNumbers(const std::vector<std::pair<int, int>>& numbers) {
...
std::map<int, FilterTimeFrameSliceByTracking_GR::GeofieldClusterInfo> FilterTimeFrameSliceByTracking_GR::analyzeGeofieldClusters(const std::map<int, std::vector<std::pair<int, int>>>& GeoIDs) {
...
bool FilterTimeFrameSliceByTracking_GR::allKeysHaveAtLeastOneCluster(const std::map<int, GeofieldClusterInfo>& geofieldClusterMap) {
...
double FilterTimeFrameSliceByTracking_GR::calculateCentroid(const std::vector<std::pair<int, int>>& cluster) {
...
void FilterTimeFrameSliceByTracking_GR::GenerateCombinations(const std::map<int, GeofieldClusterInfo>& geofieldClusterMap, int trackCount, std::vector<std::vector<std::pair<int, int>>>& combinations) {
    combinations.clear();
...
void FilterTimeFrameSliceByTracking_GR::CalculateTrackParams(const std::vector<std::pair<int, int>>& combination, std::vector<double>& trackParams) {
...
double FilterTimeFrameSliceByTracking_GR::CalculateSSR(const std::vector<std::pair<int, int>>& combination, const std::vector<double>& trackParams) {
    double SSR = 0.0;
    for (int iPlane = 0; iPlane < fNPlane; ++iPlane) {
        const double cos = fCos[iPlane];
        const double sin = fSin[iPlane];
...
bool FilterTimeFrameSliceByTracking_GR::PerformTracking(const std::map<int, GeofieldClusterInfo>& geofieldClusterMap) {
...
{
    using opt = FilterTimeFrameSliceByTracking_GR::OptionKey;
...
{
    return std::make_unique<FilterTimeFrameSliceByTracking_GR>();
}
