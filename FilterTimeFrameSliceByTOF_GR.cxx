/**
 * @file FilterTimeFrameSliceByTOF_GR.cxx
 * @brief Slice GR TimeFrame by Time of Flight for NestDAQ
...
#include <map>
#include <chrono>
#include "FilterTimeFrameSliceByTOF_GR.h"
...
using nestdaq::FilterTimeFrameSliceByTOF_GR;
namespace bpo = boost::program_options;

FilterTimeFrameSliceByTOF_GR::FilterTimeFrameSliceByTOF_GR()
: totalCalls(0), totalAccepted(0)  // Initialize counters
{
}

bool FilterTimeFrameSliceByTOF_GR::ProcessSlice(TTF& tf)
{
        ...
        for (auto* hbf : *subTimeFrame) {
            ...
            for (uint64_t i = 0; i < nData; ++i) {
                if (header->femType == SubTimeFrame::TDC64H) {
                    ...
                    // Ignore heartbeat and other control words before using ch/tdc.
...
{
    using opt = FilterTimeFrameSliceByTOF_GR::OptionKey;
...
{
    return std::make_unique<FilterTimeFrameSliceByTOF_GR>();
}

// Function section
// TOF detector settings
// Define IP addresses and channels for Start and End detectors
void FilterTimeFrameSliceByTOF_GR::AddTOFHit(uint64_t femId, int ch, int tdc) {
    if (femId == 0xc0a802a9) {
        if (ch == 9) {
...
std::vector<std::unique_ptr<int>> FilterTimeFrameSliceByTOF_GR::CalculateAveragePairs(const std::vector<std::unique_ptr<int>>& r_hits, const std::vector<std::unique_ptr<int>>& l_hits) {
...
void FilterTimeFrameSliceByTOF_GR::CalculateAndPrintTOF(const std::vector<std::unique_ptr<int>>& tof_end_averages, const std::vector<std::unique_ptr<int>>& tof_start_averages) {
...
// TOF for debug
void FilterTimeFrameSliceByTOF_GR::PrintList(const std::vector<std::unique_ptr<int>>& list, const std::string& listName) {
    std::cout << listName << " [";
...
// TOF Flag
bool FilterTimeFrameSliceByTOF_GR::CheckAllTOFConditions(int tof_start_ave, int tof_end_ave, const std::vector<std::unique_ptr<int>>& tof_start_r_hits, const std::vector<std::unique_ptr<int>>& tof_start_l_hits,
