...
#include <tuple>
#include <vector>
#include "FilterTimeFrameSliceByTOT_GR.h"
...
using nestdaq::FilterTimeFrameSliceByTOT_GR;
namespace bpo = boost::program_options;
uint64_t nestdaq::FilterTimeFrameSliceByTOT_GR::eventID = 0;

namespace {
enum CutThresholdIndex : std::size_t {
    kX1X2,
    kX1U1,
    kX1U2,
    kX2U1,
...
constexpr std::array<std::string_view, 6> kCutThresholdOptions {{
    "tot-threshold-x1-x2",
    "tot-threshold-x1-u1",
    "tot-threshold-x1-u2",
    "tot-threshold-x2-u1",
...
constexpr std::string_view kDefaultCutThreshold {"90.0"};
}


FilterTimeFrameSliceByTOT_GR::FilterTimeFrameSliceByTOT_GR()
...
void FilterTimeFrameSliceByTOT_GR::InitTask()
{
    // Call the parent class's Initask
    FilterTimeFrameSliceABC::InitTask();

    for (std::size_t i = 0; i < kCutThresholdOptions.size(); ++i) {
        const auto optionName = kCutThresholdOptions[i];
        const auto thresholdString =
            fConfig->GetValue<std::string>(optionName.data());
        std::size_t parsedLength = 0;
        try {
            fCutThresholds[i] = std::stod(thresholdString, &parsedLength);
        } catch (const std::exception&) {
            throw std::invalid_argument(
                std::string(optionName) +
                " must be a finite, non-negative number: " + thresholdString);
        }
        if (parsedLength != thresholdString.size() ||
            !std::isfinite(fCutThresholds[i]) || fCutThresholds[i] < 0.0) {
            throw std::invalid_argument(
                std::string(optionName) +
                " must be a finite, non-negative number: " + thresholdString);
        }
    ...
    std::cout << "[FilterTimeFrameSliceByTOT_GR] Pair-correlation TOT thresholds: "
              << "X1-X2=" << fCutThresholds[kX1X2]
              << ", X1-U1=" << fCutThresholds[kX1U1]
              << ", X1-U2=" << fCutThresholds[kX1U2]
              << ", X2-U1=" << fCutThresholds[kX2U1]
              << ", X2-U2=" << fCutThresholds[kX2U2]
              << ", U1-U2=" << fCutThresholds[kU1U2]
              << std::endl;
}

bool FilterTimeFrameSliceByTOT_GR::ProcessSlice(TTF& tf)
{
        ...
        for (auto* hbf : *subTimeFrame) {
            ...
            for (uint64_t i = 0; i < nData; ++i) {
                if (header->femType == SubTimeFrame::TDC64H) {
                    ...
                    // T_TDC_L/T_TDC_T are leading/trailing-edge data words.
                    // Ignore heartbeat and other control words before using ch/tot.
...
int FilterTimeFrameSliceByTOT_GR::DeterminePlane(uint64_t fem, int ch) {
    int planeId = -1;
...
bool FilterTimeFrameSliceByTOT_GR::Chargelogic(const std::map<int, std::tuple<int, int>>& chargeSums) {
    try {
...
// only modify cut value 100->75 2025/09/12
        if (
           ...
           n4 > 1 &&
           n5 > 0 &&
           ((static_cast<double>(chargeSum3) / n3) + (static_cast<double>(chargeSum2) / n2))  > fCutThresholds[kX2U1] &&
           ((static_cast<double>(chargeSum1) / n1) + (static_cast<double>(chargeSum2) / n2))  > fCutThresholds[kX1U1] &&
           ((static_cast<double>(chargeSum3) / n3) + (static_cast<double>(chargeSum4) / n4))  > fCutThresholds[kX2U2] &&
           ((static_cast<double>(chargeSum1) / n1) + (static_cast<double>(chargeSum4) / n4))  > fCutThresholds[kX1U2] &&
           ((static_cast<double>(chargeSum2) / n2) + (static_cast<double>(chargeSum4) / n4))  > fCutThresholds[kU1U2] &&
           ((static_cast<double>(chargeSum1) / n1) + (static_cast<double>(chargeSum3) / n3))  > fCutThresholds[kX1X2]
)
/*         if (
            n2 > 1 &&
            n4 > 1 &&
...
{
    using opt = FilterTimeFrameSliceByTOT_GR::OptionKey;
    ...
    options.add_options()
        ...
        (kCutThresholdOptions[1].data(),
         bpo::value<std::string>()->default_value(kDefaultCutThreshold.data()),
         "Threshold for the sum of mean TOT values in the VDC X1-U1 correlation")
        (kCutThresholdOptions[2].data(),
         bpo::value<std::string>()->default_value(kDefaultCutThreshold.data()),
         "Threshold for the sum of mean TOT values in the VDC X1-U2 correlation")
        (kCutThresholdOptions[3].data(),
         bpo::value<std::string>()->default_value(kDefaultCutThreshold.data()),
         "Threshold for the sum of mean TOT values in the VDC X2-U1 correlation")
        (kCutThresholdOptions[4].data(),
         bpo::value<std::string>()->default_value(kDefaultCutThreshold.data()),
         "Threshold for the sum of mean TOT values in the VDC X2-U2 correlation")
        (kCutThresholdOptions[5].data(),
         bpo::value<std::string>()->default_value(kDefaultCutThreshold.data()),
         "Threshold for the sum of mean TOT values in the VDC U1-U2 correlation");
}

std::unique_ptr<fair::mq::Device> getDevice(fair::mq::ProgOptions& /*config*/)
{
    return std::make_unique<FilterTimeFrameSliceByTOT_GR>();
}
